...
class CustomEmail
{
    ...
    public static function sendEmailPlaceOrderToResmy($to,$data){
        $subject = "A order has been placed";
        $postmark = new Postmark(POSTMARK_SERVER_API_TOKEN,SUPPORT_EMAIL,SUPPORT_EMAIL);
        if($postmark->to($to)->subject($subject)->html_message(EmailTemplate::placeOrderEmailToRest($data))->send()){
            return true;
        }else{
            return false;
        }
    ...
    public static function sendEmailPlaceOrderToAdmin($data){
        $subject = "New Order";
        $postmark = new Postmark(POSTMARK_SERVER_API_TOKEN,SUPPORT_EMAIL,SUPPORT_EMAIL);
        $postmark->to(ADMIN_EMAIL_1)->subject($subject)->html_message(EmailTemplate::placeOrderEmailToRest($data))->send();
        $postmark->to(ADMIN_EMAIL_2)->subject($subject)->html_message(EmailTemplate::placeOrderEmailToRest($data))->send();
        $postmark->to(ADMIN_EMAIL_3)->subject($subject)->html_message(EmailTemplate::placeOrderEmailToRest($data))->send();
        $postmark->to(ADMIN_EMAIL_4)->subject($subject)->html_message(EmailTemplate::placeOrderEmailToRest($data))->send();
    }
    
    public static function sendLuckyDrawEmailConfirmation($data){
        
        $subject = "🎁 Giveaway time! 🎊";
        
        $postmark = new Postmark(POSTMARK_SERVER_API_TOKEN,SUPPORT_EMAIL,SUPPORT_EMAIL);
        $postmark->to($data['email'])->subject($subject)->html_message(EmailTemplate::luckyDrawConfirmation($data))->send();
...
/*echo $postmark->to($toEmail)->subject($subject)->html_message(EmailTemplate::resetPassword($toEmail, $full_name,$code))
            ->send();
            $headers = "From: webmaster@example.com" . "\r\n" .
"CC: info@orderonline.hk";
           if( mail($toEmail,$subject,EmailTemplate::resetPassword($toEmail, $full_name,$code),$headers))
              return true;
            else
             return false;
            */
    ...
    public static function welcomeEmail($email,$key){
        ...
        $subject = "Welcome to OrderOnline";
